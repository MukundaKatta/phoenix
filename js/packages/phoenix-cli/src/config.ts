 ...
 *   3. `activeProfile` field in the profiles file
 *
 * Uses forgiving mode for the profiles file itself — a missing or corrupt
...
export function loadConfigFromProfile(profileName?: string): PhoenixConfig {
  ...
  const resolvedName = explicitName ?? profilesFile.activeProfile ?? undefined;
  const active = getActiveProfile(profilesFile, resolvedName);
...
export interface ResolveConfigOptions {
   ...
   * env var and the activeProfile stored in the profiles file.
   */
  profileName?: string;
}
 ...
 *   3. Active profile (from --profile, PHOENIX_PROFILE, or profiles file)
 *   4. Built-in defaults
 */
