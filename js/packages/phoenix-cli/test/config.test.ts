...
/**
 * Write a profiles file into a temp XDG_CONFIG_HOME directory so that
 * loadConfigFromProfile() / resolveConfig() can read it without touching
 * the real ~/.px/profiles.json.
 */
function writeTempProfiles(tmpDir: string, data: ProfilesFile): void {
  ...
  const profilesPath = path.join(pxConfigDir, "profiles.json");
...
describe("Configuration", () => {
  ...
  describe("loadConfigFromProfile", () => {
    ...
    it("loads the active profile from the profiles file", () => {
      writeTempProfiles(tmpDir, {
        ...
        activeProfile: "dev",
        profiles: {
          dev: { endpoint: "http://dev.example.com", apiKey: "dev-key" },
        },
      });
      const config = loadConfigFromProfile();
    ...
    it("loads profile via PHOENIX_PROFILE env var when no explicit name", () => {
      writeTempProfiles(tmpDir, {
        ...
        activeProfile: null,
        profiles: {
          staging: { endpoint: "https://staging.example.com", apiKey: "s-key" },
        },
      });
      process.env.PHOENIX_PROFILE = "staging";
      const config = loadConfigFromProfile();
      expect(config.endpoint).toBe("https://staging.example.com");
      expect(config.apiKey).toBe("s-key");
    });
    ...
    it("loads only the fields present in the profile entry", () => {
      writeTempProfiles(tmpDir, {
        ...
        profiles: { partial: { apiKey: "only-key" } },
      });
      const config = loadConfigFromProfile();
      expect(config.apiKey).toBe("only-key");
      expect(config.endpoint).toBeUndefined();
      expect(config.project).toBeUndefined();
    });
  ...
  describe("resolveConfig", () => {
    ...
    it("profile apiKey is included when no env override", () => {
      writeTempProfiles(tmpDir, {
        version: 1,
        activeProfile: "prod",
        profiles: {
    ...
    it("env var apiKey overrides profile apiKey", () => {
      writeTempProfiles(tmpDir, {
        version: 1,
        activeProfile: "dev",
        profiles: { dev: { apiKey: "profile-key" } },
      });
